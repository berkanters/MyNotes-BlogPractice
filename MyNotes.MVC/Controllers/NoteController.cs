...
namespace MyNotes.MVC.Controllers
{
    public class NoteController : Controller
    {
        ...
        {
            List<Note> notes = nm.QList().Include("Category").Include("Owner").Where(X => X.Owner.Id == CurrentSession.User.Id).OrderByDescending(s => s.ModifiedOn).ToList();
            return View(notes);
        }

        public ActionResult MyLikedNotes()
        {
            var note = lm.List(s => s.LikedUser.Id == CurrentSession.User.Id).Select(s => s.Note);
